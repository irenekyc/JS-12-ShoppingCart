...
pageContainer.addEventListener("click", (e) => {
  if (
    e.target.classList.value === "cart-icon-container" ||
    e.target.classList.value === "fas fa-shopping-cart"
  ) {
...
const updateQuantity = () => {
  ...
  if (cartQuantity == 0) {
    document
      .querySelector(".shopping-cart-content-container")
      .insertAdjacentHTML(
        "beforeend",
        ` <div class="message-container"><h3 class="message"> Your cart is currently empty </h3></div>`
      );
  ...
  document.getElementById("shopping-cart-total").innerHTML = totalPrice.toFixed(
    2
  );
};
...
productContainer.addEventListener("click", (e) => {
  if (e.target.classList.value === "product-cart") {
    ...
    cartQuantity++;
    updateCartIcon();
    const cartAddModal = document.querySelector(".modal-add-to-cart");
    if (cartAddModal) {
      cartAddModal.remove();
    }
